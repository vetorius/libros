 ...
 *
 * archivo pdf_editorial.php
 *
 * Genera un PDF con el pedido de libros para cada editorial
 *
 */
...
if ($edi->numregistros()>0) {
	...
	$sql .= "WHERE fa.id_pedido=0 and ed.id_editorial=" . $datos['id_editorial'];
	$sql .= " GROUP BY id_libro ORDER BY id_materia";
	...
	if ($base->numregistros()>0) {
		...
		$pdf->SetY(20);
		$titulo = "Pedido de libros para la editorial ".$datos['editorial'];
