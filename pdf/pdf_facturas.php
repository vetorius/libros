...
if (isset($_GET['pedido'])) {
	...
	//conexion para obtener las facturas de un pedido
	$base1 = new DBmy();
	$base1->conectar($sql_base, $sql_host, $sql_user, $sql_pass) or die ("conexion pedidos erronea");
	$base = new DBmy();
	$base->conectar($sql_base, $sql_host, $sql_user, $sql_pass) or die ("conexion factura erronea");
	
	$sql  = "SELECT f.id_factura, f.id_alumno, (a.id_curso - a.repite + 1) as curso ";
	$sql .= "FROM facturas f INNER JOIN alumnos a USING(id_alumno) ";
	$sql .= "WHERE f.id_pedido=$id_pedido ";	
	$sql .= "ORDER BY curso, a.ap1, a.ap2, a.nom";	
	$base1->consulta($sql) or die("consulta facturas de un pedido erronea");

	while ($la_factura = $base1->obtenerfila()) {
			
		$id_factura = $la_factura[0];
		$id_alumno  = $la_factura[1];
	
		// incluimos la siguiente factura
		$pdf->AddPage();
	 	
		// consulta de los datos del alumno para la cabecera
		$sql  = "SELECT id_alumno, nom, ap1, ap2, id_curso, clase_n ";
		$sql .= "FROM alumnos ";
		$sql .= "WHERE id_alumno=$id_alumno";
		
		$base->consulta($sql) or die("consulta datos alumno erronea");
	...
	// imprime los datos del alumno
			...
			$titulo .= " - " . $curso[$la_factura[2]-1] . " " . $clase[$datos['clase_n']];
			$titulo .= "\nFecha: " . date('d - m - Y');
	...
	// imprime la tabla de libros
			...
			$sql .= " and li.gratuidad=0 ORDER BY ma.tipo, li.id_materia, li.titulo";
	//		echo $sql . '<br>';
